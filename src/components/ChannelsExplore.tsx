...
export default function ChannelsExplore({
  ...
  return (
    <div id="messaging-channels-view" className="max-w-4xl mx-auto px-4 py-8 select-none">
      <button 
        onClick={onBack} 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {ALL_CHANNELS.map(chan => {
          ...
          return (
            <div 
              key={chan.id} 
              className={`border rounded-3xl p-6 bg-white shadow-xs transition-all flex flex-col justify-between ${
                isActive ? 'border-brand-yellow ring-4 ring-brand-yellow/10' : 'border-zinc-150 hover:border-zinc-300'
              }`}
            >
              <div className="space-y-2 mb-6">
                <div className="flex items-center gap-1.5">
                  <span className="text-brand-yellow font-bold text-lg">#</span>
                  <h4 className="font-bold text-brand-black text-sm">{chan.label}</h4>
                </div>
                <p className="text-zinc-500 text-[11px] font-light leading-relaxed min-h-[44px] line-clamp-3">
                  {chan.desc}
                </p>
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 bg-brand-black hover:bg-zinc-900 text-white rounded-xl text-xs font-semibold tracking-wide transition-all uppercase cursor-pointer text-center"
              >
                Enter Channel
              </button>
            </div>
